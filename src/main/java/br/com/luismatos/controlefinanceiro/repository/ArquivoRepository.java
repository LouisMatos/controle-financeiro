...
public interface ArquivoRepository extends JpaRepository<Transacao, Long> {
	...
	@Query(value = "SELECT distinct  DATE_FORMAT(cast(data_transacao as date), '%d/%m/%Y') as dataTransacoes, DATE_FORMAT(t.data_importacao_transacoes, '%d/%m/%Y %H:%i:%s') as dataImportacao FROM transacoes t", nativeQuery = true)
